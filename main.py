...
def welcome():
    print("Welcome to HAL 9000")
    hello = input("Would you like to login? Y/n ").lower()
    if hello != "n":
        login()
    else:
        print("Goodbye")
...
def new_user():
    ...
    with open("user_info.csv") as infile:
        user_data = csv.DictReader(infile, fieldnames=["username", "password", "full_name", "date_created"])
...
def login():
    ...
    with open("user_info.csv") as infile:
        user_data = csv.DictReader(infile, fieldnames=[
            "username", "password", "full_name", "fav_color", "date_created"])

        for user in user_data:
            if login_user_name.lower() == user["username"].lower():
